v1alpha1.extension_repo(name='tilt-extensions', url='https://github.com/alokkulkarni/tilt-extensions')
...
create_mysql_resource('buildpacks')

configmap_create('buildpacks-k8s', from_file=['application.yaml=./k8s/buildpacks-k8s.yaml'])
configmap_create('buildpacks-k8s-dev', from_file=['application.yaml=./k8s/buildpacks-k8s-dev.yaml'])
...
# Manage
k8s_resource('buildpacks-k8s', port_forwards=['8080:8080', '5005:5005'],
objects=['buildpacks-k8s:ConfigMap'], resource_deps=['spring-cloud-kubernetes-configserver-deployment', 'spring-cloud-kubernetes-discoveryserver-deployment', 'mysql'])